...
{
    return fuzzed_data_provider.ConsumeBool() ? fuzzed_data_provider.PickValueInArray<WeakEnumType>(all_types) : WeakEnumType(fuzzed_data_provider.ConsumeIntegral<typename std::underlying_type<WeakEnumType>::type>());
}
...
[[nodiscard]] inline int64_t ConsumeTime(FuzzedDataProvider& fuzzed_data_provider) noexcept
{
    ...
    static const int64_t time_max = ParseISO8601DateTime("12972-12-31T23:59:59Z");
