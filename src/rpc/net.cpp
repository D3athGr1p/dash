        ...
        RPCResult{
            RPCResult::Type::ARR, "", "", {{
                                              ...
                                              {{
                                                  ...
                                                  {RPCResult::Type::OBJ, "bytesrecv_per_msg", "", {{RPCResult::Type::NUM, "msg", "The total bytes received aggregated by message type\n"
...
static UniValue addnode(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{
        ...
        RPCResult{RPCResult::Type::NONE, "", ""},
        RPCExamples{
            HelpExampleCli("addnode", "\"192.168.0.6:12972\" \"onetry\"") + HelpExampleRpc("addnode", "\"192.168.0.6:12972\", \"onetry\"")},
    }
        .Check(request);
...
static UniValue disconnectnode(const JSONRPCRequest& request)
{
    RPCHelpMan{
        ...
        RPCResult{RPCResult::Type::NONE, "", ""},
        RPCExamples{
            HelpExampleCli("disconnectnode", "\"192.168.0.6:12972\"") + HelpExampleCli("disconnectnode", "\"\" 1") + HelpExampleRpc("disconnectnode", "\"192.168.0.6:12972\"") + HelpExampleRpc("disconnectnode", "\"\", 1")},
    }
        .Check(request);
...
static UniValue cleardiscouraged(const JSONRPCRequest& request)
{
    RPCHelpMan{
        ...
        RPCResult{RPCResult::Type::NONE, "", ""},
        RPCExamples{
            HelpExampleCli("cleardiscouraged", "") + HelpExampleRpc("cleardiscouraged", "")},
    }
        .Check(request);
    const NodeContext& node = EnsureAnyNodeContext(request.context);
    if (!node.banman) {
...
static UniValue getnodeaddresses(const JSONRPCRequest& request)
{
    RPCHelpMan{
        ...
        RPCExamples{HelpExampleCli("getnodeaddresses", "8") + HelpExampleRpc("getnodeaddresses", "8")},
    }
        .Check(request);
    const NodeContext& node = EnsureAnyNodeContext(request.context);
    if (!node.connman) {
...
static UniValue addpeeraddress(const JSONRPCRequest& request)
{
    RPCHelpMan{
        ...
        RPCExamples{
            HelpExampleCli("addpeeraddress", "\"1.2.3.4\" 12972") + HelpExampleRpc("addpeeraddress", "\"1.2.3.4\", 12972")},
    }
        .Check(request);
